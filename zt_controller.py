    ...
    while True:
        ...
        for ew_src, ew_dsts in east_west_map.items():
            ...
            if not blocked_dsts:
                ...
                # Clear flag so scoring doesn't penalise this device
                if ew_container in dh:
                    dh[ew_container]["east_west"] = False
                # Add to ew_containers so per-device loop skips setting it True
        ...
        for ip, info in known_devices.items():
            ...
            if ip in east_west_map and key not in ew_containers:
                # ew_containers includes whitelisted sources — don't re-flag them
